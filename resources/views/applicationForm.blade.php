...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <style>
            body {
                ...
                background: -webkit-linear-gradient(to bottom, #1b1b31, #23233d 50vh, #0f0c29);  /* Chrome 10-25, Safari 5.1-6 */
                background: linear-gradient(to bottom, #1c1646, #18143b 75vh, #18143a); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
                min-height: 100vh;
            ...
            .header::before {
                ...
                background: url({{ asset('images/brand-image.svg') }}) right center no-repeat;
    ...
    <body class="antialiased container p-0">
        ...
        <div class='d-flex flex-column h-100 bg-light px-5 py-4'>
            <section>
            ...
            <section class='pt-4 d-flex flex-column'>
                <h3 class='h1'> Apply now! </h3>
                <form class='col-12 mt-3'>
                    @foreach ($form_fields as $field)
                        <div class='form-group'>
                            <label for="{{ $field['text'] }}"> {{ $field['text'] }} </label>
                            <input type="{{ $field['type'] }}" id="{{ $field['text'] }}"
                            class="@if ($field['type'] !== 'file') form-control @endif" />
                        </div>
                    @endforeach
                </form>
            </section>
